//
//  Theme.swift
//  DiscordBotApp
//
//  Created by speedy on 2024/12/24.
//

import SwiftUI

enum Theme {
    static let primaryBackground = Color(hex: "313338")
    static let secondaryBackground = Color(hex: "2B2D31")
    static let accent = Color(hex: "5865F2")
    static let textPrimary = Color.white
    static let textSecondary = Color(hex: "B5BAC1")
    static let inputBackground = Color(hex: "1E1F22")
    static let success = Color(hex: "23A559")
    static let error = Color(hex: "F23F42")
    
    static let cornerRadius: CGFloat = 8
    static let glowRadius: CGFloat = 15
    static let glowOpacity: CGFloat = 0.5
}

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3:
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6:
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8:
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (255, 0, 0, 0)
        }
        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue:  Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}
